...
**Example status — runnable:** the record, schema, and encoded-inspection cells
execute as a small inference-path walkthrough, not a training job.
...
PRICE_ADDRESS = rf.Address("order", "line_items", "price")
encoded = model.encode([record])
```

Display the batch itself for a compact inventory of leaf addresses, structural
axis names, and state shapes:
...
The summary reports that metadata is present but deliberately hides its raw
values. Select one observation and an exact leaf address when you need the
TensorField preview:
...
encoded[0][PRICE_ADDRESS]
```

`encoded[0]` selects the first observation across every leaf in the encoded
batch. Use `encoded[:1]` instead when downstream code should retain the batch
axis. Most application code should not depend on these internal tensor layouts;
these compact displays are useful for checking that preprocessing and queries
found the expected values and shapes.
