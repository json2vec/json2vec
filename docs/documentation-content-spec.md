...
| Hash semantics | The type uses static CPU hashes plus device salts derived from Lightning `global_step` rather than a learned vocabulary. | Describe equality across fields with matching salting policies, the optimizer-step salt sequence across epochs, shared salts under gradient accumulation and frozen validation steps, always-unsalted inference, correspondingly salted quantized reconstruction targets, the persistent but overfitting-prone `deterministic=True` opt-out, and the correspondence lost when sibling branches pool independently. |
